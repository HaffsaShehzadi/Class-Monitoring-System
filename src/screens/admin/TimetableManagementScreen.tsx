...
export default function TimetableManagementScreen({ onBack, onNavigate }: any) {
  ...
  const [timetable] = useState<any[]>([
    { id: 1, dept: 'IT', sem: '2nd', day: 'Monday', period: 1, teacher: 'Hassan Raza', code: 'UE-272', room: 'R58', section: '[1-4]' },
    { id: 2, dept: 'IT', sem: '2nd', day: 'Monday', period: 2, teacher: 'Mohsin Raza', code: 'GENG-201', room: 'R58', section: '[1-2]' },
    { id: 3, dept: 'IT', sem: '4th', day: 'Monday', period: 1, teacher: 'Hassan Raza', code: 'CC-213L', room: 'R59', section: '[1-2]' },
  ...
  const sortSemesters = (sems: string[]) =>
    [...sems].sort((a, b) => (parseInt(a) || 0) - (parseInt(b) || 0));
  ...
  const handleRemoveDepartment = (deptToRemove: string) => {
    Alert.alert('Remove Department', `Are you sure you want to remove ${deptToRemove}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Remove', style: 'destructive', onPress: () => setDepartments(departments.filter(d => d !== deptToRemove)) }
    ]);
  };

  const handleAddSemester = () => {
    if (newSemName.trim()) {
      if (!semesters.includes(newSemName.trim())) {
        setSemesters(sortSemesters([...semesters, newSemName.trim()]));
        setNewSemName('');
  ...
  const handleRemoveSemester = (semToRemove: string) => {
    Alert.alert('Remove Semester', `Are you sure you want to remove ${semToRemove}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Remove', style: 'destructive', onPress: () => setSemesters(semesters.filter(s => s !== semToRemove)) }
    ]);
  };
  ...
  const handleSavePeriod = () => {
    ...
    setPeriods(periods.map(p => p.id === editingPeriod.id ? { ...p, time: `${periodStartTime} - ${periodEndTime}` } : p));
  ...
  const handleDeletePeriod = (periodId: number) => {
    Alert.alert('Remove Period', 'Are you sure you want to remove this period?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Remove', style: 'destructive', onPress: () => setPeriods(periods.filter(p => p.id !== periodId)) }
    ]);
  };
        ...
        <ScrollView contentContainerStyle={styles.deptListContent}>
          {departments.length === 0 ? (
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No departments added yet</Text>
            </View>
          ) : (
            departments.map(dept => (
              <View key={dept} style={styles.deptCard}>
                ...
                <TouchableOpacity style={styles.deleteDeptBtn} onPress={() => handleRemoveDepartment(dept)}>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.modalInputRow}>
                <TextInput style={styles.modalInput} placeholder="Enter new department" value={newDeptName} onChangeText={setNewDeptName} placeholderTextColor="#999" />
    ...
    <SafeAreaView style={styles.container}>
            ...
            {semesters.map(sem => (
              <View key={`${selectedDepartment}-${sem}`} style={styles.row}>
                ...
                {periods.map(p => {
                  ...
                  return (
                    <TouchableOpacity key={p.id} style={[styles.dataCell, cls ? styles.filledCell : styles.emptyCell]} onPress={() => handleCellPress(selectedDepartment!, sem, p.id, cls)} activeOpacity={0.7}>
                      {cls ? (
      ...
      <Modal visible={showSemModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalList}>
              {semesters.map(sem => (
                <View key={sem} style={styles.listItem}>
                  ...
                  <TouchableOpacity style={styles.deleteBtn} onPress={() => handleRemoveSemester(sem)}>
            ...
            <View style={styles.modalInputRow}>
              <TextInput style={styles.modalInput} placeholder="Enter new semester" value={newSemName} onChangeText={setNewSemName} placeholderTextColor="#999" />
      ...
      <Modal visible={showPeriodModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{editingPeriod ? 'Edit Period' : 'Add Period'}</Text>
              <TouchableOpacity onPress={() => setShowPeriodModal(false)}>
                <MaterialCommunityIcons name="close" size={24} color="#1A237E" />
              </TouchableOpacity>
            </View>
            <Text style={styles.inputLabel}>Start Time</Text>
            <TextInput style={styles.periodInput} placeholder="e.g., 08:30" value={periodStartTime} onChangeText={setPeriodStartTime} placeholderTextColor="#999" />
            <Text style={styles.inputLabel}>End Time</Text>
            <TextInput style={styles.periodInput} placeholder="e.g., 09:15" value={periodEndTime} onChangeText={setPeriodEndTime} placeholderTextColor="#999" />
            {editingPeriod && (
            ...
            <TouchableOpacity style={styles.savePeriodBtn} onPress={editingPeriod ? handleSavePeriod : handleCreatePeriod}>
              <Text style={styles.savePeriodText}>{editingPeriod ? 'Save Changes' : 'Add Period'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
